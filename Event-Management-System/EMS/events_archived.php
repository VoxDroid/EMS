...
<div class="py-5 flex-grow-1" style="background-color: #1c2331">
    <div class="container">
        ...
        <div class="row">
            <?php
            // Fetch archived events count
            $queryArchivedEventsCount = "SELECT COUNT(*) AS count FROM events WHERE status IN ('completed', 'denied')";
            $stmtArchivedEventsCount = $pdo->query($queryArchivedEventsCount);
            $archivedEventCount = $stmtArchivedEventsCount->fetch(PDO::FETCH_ASSOC)['count'];

            // Define items per page for archived section
            $archivedItemsPerPage = 50;

            // Calculate total pages for archived section
            $archivedTotalPages = getTotalPages($archivedEventCount, $archivedItemsPerPage);

            // Fetch archived events with pagination
            $archivedCurrentPage = isset($_GET['archived_page']) ? max(1, intval($_GET['archived_page'])) : 1;
            $archivedOffset = ($archivedCurrentPage - 1) * $archivedItemsPerPage;
            $queryArchivedEvents = "SELECT e.*, u.username, u.profile_picture 
                                    FROM events e
                                    JOIN users u ON e.user_id = u.id
                                    WHERE e.status IN ('completed', 'denied')
                                    ORDER BY e.date_requested ASC 
                                    LIMIT $archivedOffset, $archivedItemsPerPage";
            $stmtArchivedEvents = $pdo->query($queryArchivedEvents);

            if ($stmtArchivedEvents->rowCount() > 0) {
                while ($event = $stmtArchivedEvents->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo '<p class="text-white">No archived events found.</p>';
                echo '</div>';
            }

            // Display pagination controls for archived section
            if ($archivedEventCount > $archivedItemsPerPage) {
                echo '<div class="col-md-12">';
                echo '<nav aria-label="Page navigation example">';
                echo '<ul class="pagination justify-content-center">';
                echo '<li class="page-item ' . ($archivedCurrentPage == 1 ? 'disabled' : '') . '">';
                echo '<a class="page-link custom-page-link" href="?archived_page=' . max(1, $archivedCurrentPage - 1) . '" aria-label="Previous">';
                echo '<span aria-hidden="true">&laquo;</span>';
                echo '<span class="sr-only">Previous</span>';
                echo '</a>';
                echo '</li>';
                for ($i = 1; $i <= $archivedTotalPages; $i++) {
                    echo '<li class="page-item ' . ($archivedCurrentPage == $i ? 'active' : '') . '"><a class="page-link custom-page-link" href="?archived_page=' . $i . '">' . $i . '</a></li>';
                }
                echo '<li class="page-item ' . ($archivedCurrentPage == $archivedTotalPages ? 'disabled' : '') . '">';
                echo '<a class="page-link custom-page-link" href="?archived_page=' . min($archivedTotalPages, $archivedCurrentPage + 1) . '" aria-label="Next">';
